...
function create(options) {
    ...
    create({
        ...
        id: [
            'id-1',
            'id-2',
        ], // Elementimize eklemek istediğimiz idler

        parent: '', // Hangi elementin içerisine eklenecek -> querySelector() (String ve obje destekler)
    ...
    */
    ...
    let ids = options.id !== undefined ? (typeof(options.id === 'object') ? options.id.join(' ') : options.id) : '';
    createElement.classList.add(...classes);
    createElement.setAttribute('id', ids);
    createElement.innerHTML = options.content ?? null;
    if(options.parent !== undefined && typeof(options.parent) === 'object'){
        let objParent = options.parent;
        objParent.append(createElement);
    }else if(options.parent !== undefined && typeof(options.parent) === 'string'){
